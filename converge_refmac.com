#! /bin/tcsh -f
#    
#    auto-converging refmac5 script        -James Holton     1-15-22
#
...
set mypid = $$
foreach retry ( 1 2 3 )
  set pids = `ps -fu $USER | tee debug1.log | egrep converge_refmac.com | egrep -v " egrep -v | grep -E |^UID| srun converge" | awk -v pid=$mypid '$2!=pid{print "/proc/"$2"/cwd"}'`
...
if( "$max_runtime" != "0" ) then
    ...
    ( sleep $max_runtime >& /dev/null ; ( echo "stop Y" > refmac_stop.txt ) ) &
    ps -fH --ppid=$!
...
EOF
    ...
    if($?max_diverge && $?minR_n) then
        ...
        if($test) then
            set DIVERGING
            echo "things are getting worse! salvaging best R ; this will be the last time. "
